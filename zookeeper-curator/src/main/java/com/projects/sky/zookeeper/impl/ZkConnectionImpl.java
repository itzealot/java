...
import com.projects.sky.zookeeper.ZkPath.PathFilter;
import com.projects.sky.zookeeper.ZkPath.ZkACL;
	...
	public <T extends Serializable> T getData(ZkPathImpl path) {
		...
		try {
			if (path.watch == null)
				...
				return Commons.read(client.getData().usingWatcher(path.hook).forPath(path.getPath()));
		} catch (Exception e) {
			e.printStackTrace();
		}
