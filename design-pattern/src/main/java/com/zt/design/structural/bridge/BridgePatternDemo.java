...
/**
 * 7. 桥接模式.<br />
 * 7.1 意图：将抽象部分与实现部分分离，使它们都可以独立的变化。
 * 
 * 7.2 主要解决：在有多种可能会变化的情况下，用继承会造成类爆炸问题，扩展起来不灵活。
 * 
 * 7.3 何时使用：实现系统可能有多个角度分类，每一种角度都可能变化。
 * 
 * 7.4 如何解决：把这种多角度分类分离出来，让它们独立变化，减少它们之间耦合。
 * 
 * 7.5 关键代码：抽象类依赖实现类。
 * 
 * 7.6 应用实例：<br />
 * 1、猪八戒从天蓬元帅转世投胎到猪，转世投胎的机制将尘世划分为两个等级，即：灵魂和肉体，前者相当于抽象化，后者相当于实现化。生灵通过功能的委派，
 * 调用肉体对象的功能，使得生灵可以动态地选择。 <br />
 ...
 * 3、实现细节对客户透明。
 * 
 * 7.8 缺点：桥接模式的引入会增加系统的理解与设计难度，由于聚合关联关系建立在抽象层，要求开发者针对抽象进行设计与编程。
 * 
 * 7.9 使用场景：<br />
 ...
 * 
 * 7.10 注意事项：对于两个独立变化的维度，使用桥接模式再适合不过了。
 * 
 * @author zengtao
 *
 */
