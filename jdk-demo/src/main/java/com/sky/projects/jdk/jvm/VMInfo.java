...
public final class VMInfo {
	...
	public static boolean isSunOsMBean(OperatingSystemMXBean operatingSystem) {
		return "com.sun.management.UnixOperatingSystem".equals(operatingSystem.getClass().getName());
	}
	...
	private class ProcessCpuStatus {
		// 百分比的值 比如30.0 表示30.0%
		float maxDeltaCpu = -1; // 最大cpu使用率
		float minDeltaCpu = -1; // 最小cpu使用率
		float curDeltaCpu = -1; // 当前cpu使用率
		float averageCpu = -1; // cpu平均使用率
	...
	/**
	 * GC 状态信息，如GC次数(最大、最小、当前、平均)，GC时间(最大、最小、当前、平均)
	 */
	...
	/**
	 * 内存状态信息，如：初始化大小，最大大小，使用大小，使用百分比，最大使用，最大使用百分比
	 */
